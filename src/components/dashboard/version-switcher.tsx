...
export function VersionSwitcher() {
  ...
  if (isLoading) {
    return (
      <div className="h-9 w-44 animate-pulse rounded-xl bg-slate-800/50" />
    );
  }
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="flex items-center gap-2.5 rounded-xl border border-white/10 bg-slate-900/80 px-3 py-1.5 text-xs font-semibold text-slate-200 transition-all hover:bg-slate-900 hover:border-primary/50 shadow-sm backdrop-blur-sm"
        aria-label="Select product version"
      >
        <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-lg bg-primary/20">
        ...
        <ChevronDown className={`h-3.5 w-3.5 text-slate-400 stroke-[1.5] transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} />
      </button>

      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 top-full z-50 mt-2 w-72 overflow-hidden rounded-xl border border-white/10 bg-slate-900/95 shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-200 text-slate-200">
          <div className="border-b border-white/10 px-4 py-2.5 bg-black/20">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">Select Audit Context</h4>
            <p className="text-[10px] text-slate-500 mt-0.5">Isolates RAG evidence and scores of the corresponding version.</p>
          </div>
          ...
          <div className="p-1 space-y-1 max-h-80 overflow-y-auto">
            ...
            >
              <div className="flex items-center justify-between font-semibold">
                ...
                <span className="rounded bg-slate-500/20 px-1 py-0.5 text-[8px] uppercase tracking-wider text-slate-400">Organization</span>
              </div>
              <p className="text-[10px] text-slate-500">Corporate security policies (ISO 27001, HR, etc.)</p>
            </button>
