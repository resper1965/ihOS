    ...
    try {
      const res = await fetch("/api/compliance/report", {
        ...
        body: JSON.stringify({
          ...
          title: `ISO-27001 Gap Analysis Report - ${new Date().toLocaleDateString("en-US")}`,
        }),
      });
  ...
  return (
    <div className="w-full space-y-8">
      <PageTitleRegistrar
        ...
        subtitle="Generate, view, and download detailed reports generated by the GRC intelligence."
      ...
      <div className="glass-card p-6 space-y-6">
        ...
        {loading ? (
          ...
          <div className="divide-y divide-white/5">
            {filtered.map((rep) => (
              <div key={rep.id} className="flex flex-col py-4 gap-3 md:flex-row md:items-center md:justify-between hover:bg-white/[0.01] px-2 rounded-lg transition-colors">
                ...
                <div className="flex flex-wrap items-center gap-3 self-end md:self-auto">
                  <span className="text-xs text-text-muted">Generated: {formatDate(rep.createdAt)}</span>
                  <Badge variant={rep.status === "ready" ? "success" : "warning"} dot>
                    {rep.status === "ready" ? "Available" : "Generating..."}
                  </Badge>
            ...
            {filtered.length === 0 && (
              <p className="text-center py-6 text-sm text-text-muted">No reports found.</p>
            )}
          </div>
        )}
      </div>
