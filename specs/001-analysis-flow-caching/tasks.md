...
- [ ] T053 Investigate the pre-existing failing test `tests/unit/test_threat_modeling_post.test.ts > "should run engine generate when parameters are valid"` (fails on `main` before this feature too — the shared admin-client test mock resolves `.single()` to `{data: null}`, which the route correctly treats as an insert failure). Not caused by this feature; left as-is to avoid masking a real insert-failure code path.
